...
def deploy(hosts, tagName) {
    ...
    withCredentials([usernamePassword(credentialsId: 'aliyun-registry-admin', passwordVariable: 'password', usernameVariable: 'username')]) {
        sh """
            ...
            docker pull 39.100.108.229/library/nginx16phpfpm73:${tagName}
            sleep 1
            docker run --name nginx16phpfpm73-${tagName} -p 8080:80 -d ${newImageName}:${tagName}
        """
    }
}