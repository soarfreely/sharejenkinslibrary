...
//    withCredentials([usernamePassword(credentialsId: 'aliyun-registry-admin', passwordVariable: 'password', usernameVariable: 'username')]) {
        sh """
            ...
            sleep 1
        """
        sh "docker rm -f ${domain} > /tmp/error 2>&1"
        sh "docker run --name ${domain} -p ${nginxProxyPort}:80 -d ${imageRepoUri}/${domain}:${tagName}"
        sh "docker rmi -f ${imageRepoUri}/${domain}:${tagName}"
//    }
}
